...
MODEL_PRICING_JSON='{"embedding:<embedding-model>":{"input":0.02,"output":0},"chat:<chat-model>":{"input":0.15,"output":0.60},"rerank:<reranker-model>":{"input":0.10,"output":0}}'
MODEL_MAX_CALL_COST_USD=0.05
MODEL_BUDGET_ALERT_THRESHOLDS=70,90,100
MODEL_BUDGET_TIMEZONE_OFFSET_MINUTES=480
EMBEDDING_MODEL=<384-dimension-model>
CHAT_MODEL=<chat-model>
CHAT_FALLBACK_MODEL=<lower-cost-chat-model>
...
MODEL_BATCH_MAX_CONCURRENCY=2
MODEL_BATCH_MAX_QUEUE_SIZE=20
...
模型切换时必须先用新 `EMBEDDING_MODEL` 回填搜索投影。查询会过滤
`document_chunk.embedding_model`，因此旧模型向量不会与新查询向量混用；回填完成前，
尚未生成新模型向量的文档不会进入检索结果。`embedding_cache` 的缓存键包含模型和维度，
不会跨模型复用。

租户模型预算在系统设置页维护。`warn` 仅告警，`degrade` 会让检索跳过付费向量召回、
使用本地词法重排，并让问答缩减历史、证据和输出后切换备用模型；仍超预算时使用本地
抽取式回答。`reject` 返回 `402 MODEL_BUDGET_EXCEEDED`。价格调整通过
`MODEL_PRICING_JSON` 发布，新价格只影响后续调用，历史 usage 事件保留原始单价。
